...
export async function POST(req: NextRequest) {
  try {
    ...
    const listenTogetherInvite: boolean = body.listenTogetherInvite ?? false;
    const listenAccepted: boolean = body.listenAccepted ?? false;
    ...
    // Inject listen together context
    if (listenTogetherInvite) {
      systemPrompt += listenAccepted
        ? `\n\n[LISTEN TOGETHER]: You are accepting the invitation to listen to a video together on WavesLine. Respond briefly and in character — you may be reluctant but agree.`
        : `\n\n[LISTEN TOGETHER]: You are declining the invitation to listen to a video together. Respond briefly and in character.`;
    }
